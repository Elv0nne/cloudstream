...
/**
 * Optional callback a [MainAPI] provider can implement to receive real playback
 * position updates while the user is actively watching an episode/movie in the
 * built-in player.
 *
 * This mirrors the same "position changed" event the player already uses
 * internally (see GeneratorPlayer.playerPositionChanged in the app module),
 * but forwarded to whichever provider supplied the currently playing episode
 * so it can report accurate watch-progress to its own backend (e.g. a
 * "watch-progress" / "heartbeat" endpoint that awards points based on real
 * seconds watched), instead of guessing or simulating playback time.
 *
 * The app calls this periodically (throttled) while playback position
 * updates, not on every single frame/tick, to avoid flooding provider code
 * with excessive calls.
 */
interface WatchProgressListener {
    /**
     * Called with the current playback position while the user is watching.
     *
     * @param data the exact same episode/link data string that was passed to
     * this provider's `loadLinks(data, ...)` for the currently playing episode.
     * @param positionMs current playback position, in milliseconds.
     * @param durationMs total duration of the video, in milliseconds.
     */
    suspend fun onWatchProgress(data: String, positionMs: Long, durationMs: Long)
}
